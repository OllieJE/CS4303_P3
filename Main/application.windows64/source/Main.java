...
class Gate extends Interactable {
  ...
  Gate(int x, int y, String colour, int edge, float tile_size, float proportionalSize, float shift) {
    super(x, y, tile_size, proportionalSize, shift);
    int[] colourRGB = GATE_COLOURS.get(colour);
    colourString = colour;
    ...
    for (String s : entity_data_list) {
      ...
      switch (entity_data[0]) {
        ...
        case "s":
          ...
          float startTime = Float.parseFloat(entity_data[5]);
          interactables.add(new Spikes(x_pos, y_pos, tile_size, seconds_alive, seconds_delay, SPIKES_PROPORTION, horizontalShift, startTime));
          break;
        case "o":
...
class LevelEditor {
  ...
  public void placeEntity() {
    ...
    if (changing == null) {
      ...
      case 10:
        // spikes
        Interactable spikes = new Spikes((int)snap1.x, (int)snap1.y, tile_size, 2f, 2f, SPIKES_PROPORTION, horizontalShift, 0f);
...
class LevelSelect {
  ...
  LevelSelect() {
    ...
    pages = new String[(int)((float)pathnames.length/10.0f)+1][10];
...
class Player extends Rigid_Body {
  ...
  Player(int x, int y, float m, Friction f, int facing, float tile_size, float proportionalSize, float shift) {
    ...
    jumpTime = fps*1.5f;
    jumpDistance = tile_size*2.1f;  // jump two tiles
